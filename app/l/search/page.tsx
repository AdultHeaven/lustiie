...
type ThreadRow = {
  id: number; slug: string; title: string; body?: string | null;
  created_at?: string; views?: number | null; replies?: number | null;
...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    if (currentQ !== query) router.replace(nextUrl);
      ...
      <section>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-20 rounded-2xl border border-neutral-800/70 bg-neutral-950/60 animate-pulse" />
          ))}
        </div>
      </section>
      <section>
        ...
        <div className="space-y-3">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-24 rounded-2xl border border-neutral-800/70 bg-neutral-950/60 animate-pulse" />
          ))}
        </div>
      </section>
    </div>
...
function Results({ q, res, isTagMode }: { q: string; res: ResultPayload; isTagMode: boolean }) {
  return (
    <div className="space-y-8">
      {/* Matching tags (only visible when tag search or tags matched) */}
...
function highlight(text?: string, q?: string) {
  ...
  try {
    ...
    return parts.map((part, i) =>
      re.test(part) ? (
        <mark key={i} className="rounded bg-fuchsia-600/20 px-1 py-0.5 text-fuchsia-200">
          {part}
        </mark>
      ) : (
