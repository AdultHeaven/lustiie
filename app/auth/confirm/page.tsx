...
//     // default to magiclink; override if you passed a different type in the URL
...
//     if (!token_hash) {
//       setMsg('Invalid link: missing token.');
//       return;
//     }
...
//       if (error) {
//         setMsg(`Could not confirm: ${error.message}`);
//         return;
//       }
...
export default function ConfirmPage() {
  ...
  const [phase, setPhase] = useState<'verifying'|'need-username'|'done'|'error'>('verifying');
  ...
  useEffect(() => {
    const token_hash = params.get('token_hash') || '';
    const type = (params.get('type') as 'magiclink'|'signup'|'recovery'|'email_change'|'invite') || 'magiclink';
